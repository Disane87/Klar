...
export type KlarMetricTileTone = 'neutral' | 'success' | 'danger';
 ...
 *
 * Tone:
 *   - neutral (default): plain --line border, --fg value
 *   - success: success-tinted gradient + --success value
 *   - danger:  danger-tinted gradient + --danger value
 *
 * `accent` is the legacy boolean shorthand for success.
 */
@Component({
  ...
  template: `
    <div
      ...
      <span
        ...
        [ngClass]="valueClass() || valueToneClass()">
...
export class KlarMetricTileComponent {
  ...
  readonly tone       = input<KlarMetricTileTone>('neutral');
  ...
  protected readonly effectiveTone = computed<KlarMetricTileTone>(() => {
    const t = this.tone();
    if (t !== 'neutral') return t;
    return this.accent() ? 'success' : 'neutral';
  });

  protected readonly frameClasses = computed(() => {
    switch (this.effectiveTone()) {
      case 'success':
        return 'border-(--success)/40 bg-[radial-gradient(120%_80%_at_100%_0%,oklch(from_var(--success)_l_c_h_/_0.10),transparent_60%),var(--bg-1)]';
      case 'danger':
        return 'border-(--danger)/40 bg-[radial-gradient(120%_80%_at_100%_0%,oklch(from_var(--danger)_l_c_h_/_0.10),transparent_60%),var(--bg-1)]';
      default:
        return 'border-(--line)';
    }
  });

  protected readonly valueToneClass = computed(() => {
    switch (this.effectiveTone()) {
