...
@Component({
  ...
  template: `
    <div
      class="flex flex-col gap-2 rounded-lg border bg-(--bg-1) px-5 py-4 relative overflow-hidden h-full"
      [ngClass]="accent()
        ? 'border-(--success)/40 bg-[radial-gradient(120%_80%_at_100%_0%,oklch(from_var(--success)_l_c_h_/_0.10),transparent_60%),var(--bg-1)]'
        : 'border-(--line)'">
      <span class="text-[10px] uppercase tracking-[0.14em] font-medium text-(--fg-2) truncate">
        {{ label() }}
      </span>
      <span
        class="font-(family-name:--font-display) text-[28px] leading-none font-normal tracking-[-0.02em] tabular-nums truncate"
        [ngClass]="valueClass() || (accent() ? 'text-(--success)' : 'text-(--fg)')">
        <ng-content select="[slot=value]">{{ value() }}</ng-content>
      </span>
      @if (sub() || hasSubSlot) {
        <span class="text-[11px] font-(family-name:--font-mono) text-(--fg-2) flex items-center gap-1.5 truncate">
