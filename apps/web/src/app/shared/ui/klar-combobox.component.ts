  ...
  template: `
    <brn-popover #popover="brnPopover" [sideOffset]="4" align="start">
      <button
        ...
        class="flex h-8 w-full items-center justify-between gap-2 rounded border border-(--border) bg-(--surface) px-3 text-base text-(--text) scheme-dark transition-colors hover:bg-(--surface-2) disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:border-(--color-ring) data-[state=open]:border-(--color-ring)"
      >
